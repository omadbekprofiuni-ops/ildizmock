...
type Stats = {
  ...
  recent_students: { username: string; name: string; org_name: string | null; org_slug: string | null; created_at: string }[]
  ...
  return (
    <SuperAdminLayout>
      <header className="flex items-center justify-between border-b bg-white px-8 py-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Hello, Super Admin</h1>
          <p className="text-sm text-muted-foreground">ILDIZmock platform management</p>
        </div>
      ...
      <div className="space-y-6 p-8">
        ...
        {q.isError && <p className="text-destructive">Failed to load statistics.</p>}
        {q.data && (
          <>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <Card>
                <CardContent className="p-6">
                  <h2 className="mb-4 text-base font-semibold">Centers status</h2>
                  <ul className="space-y-2 text-sm">
                    ...
                    <StatusRow label="Plan expired" count={q.data.orgs_by_status.expired} dot="bg-rose-500" />
              ...
              <Card>
                <CardContent className="p-6">
                  <h2 className="mb-4 text-base font-semibold">Plans expiring soon</h2>
                  {q.data.soon_expiring.length === 0 ? (
                    <p className="text-sm text-muted-foreground">No plans expiring within 7 days.</p>
                  ) : (
                    <ul className="space-y-2 text-sm">
                      {q.data.soon_expiring.map((o) => (
                        <li key={o.id} className="flex items-center justify-between">
                          <span>{o.name} · <span className="text-muted-foreground">{o.plan_name}</span></span>
                          <span className="font-mono text-xs text-rose-700">{o.days_remaining} days</span>
                        </li>
                      ))}
                    </ul>
                  )}
                ...
                {q.data.recent_students.length === 0 ? (
                  ...
                  <ul className="divide-y">
                    {q.data.recent_students.map((s) => (
                      <li key={s.username} className="flex items-center justify-between px-6 py-3 text-sm">
                        <div>
